...
def set_appointments(request):
    ...
    try:
        ...
        # 기존 예약 삭제 (테스트 초기화)
        Appointment.objects.filter(user_id=user_id).delete()
...
def reset_test_data(request):
    ...
    try:
        ...
        # PatientState와 StateTransition은 trigger_source가 'Cypress Test'인 것만 삭제
        PatientState.objects.filter(user_id=user_id).delete()  # 이건 하나만 있으니 삭제 가능
