...
export default function ProgressBar(props) {
  ...
  // 현재 진행 중인 단계 찾기
  const currentStepIndex = appointments.findIndex(apt => apt.status === 'ongoing');
  const nextStepIndex = currentStepIndex === -1 
    ? appointments.findIndex(apt => apt.status === 'pending' || apt.status === 'waiting')
    : -1;

  // 완료된 마지막 단계 인덱스
  const lastCompletedIndex = appointments.reduce((lastIndex, apt, index) => {
    return apt.status === 'completed' ? index : lastIndex;
  }, -1);
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-between">
          {appointments.map((appointment, index) => {
            ...
            const isPending = appointment.status === 'pending' || appointment.status === 'waiting';
              ...
              >
                ...
                <div className="mt-3 text-center max-w-[80px] sm:max-w-none">
                  <p className={`
                    ...
                    ${isCompleted || isOngoing || isCurrent 
                      ? 'text-gray-900' 
                      : 'text-gray-500'
                    }
