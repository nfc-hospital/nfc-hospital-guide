...
const InteractiveMapViewer = ({ 
  mapName,  // 기존 prop (단순 버전)
  mapFileName,  // 새로운 prop (복잡 버전)
  highlightDepartment, 
  onDepartmentClick, 
  showRoute = false,
  mapUrl,  // 추가 prop
  highlightZoneId,  // 추가 prop
  onRoomSelect  // 추가 prop
}) => {
  ...
  // props 호환성을 위한 통합 처리
  const finalMapName = mapFileName || mapName;
  const finalHighlight = highlightDepartment || highlightZoneId;
  const finalClickHandler = onDepartmentClick || onRoomSelect;
  const finalMapUrl = mapUrl;
  ...
  useEffect(() => {
    if (!finalMapName && !finalMapUrl) return;
    ...
    const fetchMap = async () => {
      ...
      try {
        ...
        if (finalMapUrl) {
          ...
          // mapName이 제공된 경우 mapService 사용하거나 직접 경로
          try {
            svgText = await mapService.getMapSvgContent(finalMapName);
          } catch (serviceError) {
            // mapService 실패시 직접 경로로 시도
            const mapPath = `/images/maps/${finalMapName}.svg`;
        ...
        // 상호작용 요소 설정 (클릭 이벤트가 있는 경우에만)
        if (finalClickHandler) {
        ...
        // 하이라이트 처리
        if (finalHighlight) {
          const targetElement = svgElement.getElementById(finalHighlight);
          if (targetElement) {
            // 강조 스타일 적용
        ...
        // 범례 추가 (고급 기능이 활성화된 경우에만)
        if (showRoute || finalHighlight) {
    ...
    fetchMap();
  }, [finalMapName, finalMapUrl, finalHighlight, finalClickHandler, showRoute]);

  // 현재 위치 표시 useEffect (LocationStore 기능)
  useEffect(() => {
    ...
    if (currentCoordinateLocation.isSet && 
        currentCoordinateLocation.mapId === finalMapName) {
  ...
  }, [svgDoc, svgElement, currentCoordinateLocation, finalMapName, showRoute]);

  // 경로 표시 useEffect (90도 직각 경로)
  useEffect(() => {
    if (!svgDoc || !svgElement || !showRoute) return;
    ...
    if (routeInfo.isActive && routeInfo.coordinates.length > 1) {
      ...
      // 90도 직각 경로 라인 생성
      const pathString = generateOrthogonalPath(routeInfo.coordinates);
